    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        styleSheets.forEach((sheet) => {
            if ('cssRules' in sheet) {
                try {
                    toArray(sheet.hasOwnProperty('cssRules')).forEach((item, index) => {
                    ...
                    if (sheet.href != null) {
                        deferreds.push(fetchCSS(sheet.href)
                            .then((metadata) => metadata ? embedFonts(metadata, options) : '')
                            .then((cssText) => parseCSS(cssText).forEach((rule) => {
                            inline.insertRule(rule, sheet.hasOwnProperty('cssRules')
                                ? sheet.cssRules.length
                                : 0);
                        }))
                            .catch((err) => {
        ...
        return Promise.all(deferreds).then(() => {
            ...
            styleSheets.forEach((sheet) => {
                if ('cssRules' in sheet) {
                    try {
                        toArray(sheet.hasOwnProperty('cssRules')).forEach((item) => {
                            ret.push(item);
                        });
                    }
                    catch (e) {
